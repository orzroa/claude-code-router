...
export const createServer = async (config: any): Promise<any> => {
  ...
  app.get("/api/usage/summary", async (req: any, reply: any) => {
    try {
      const queryObj: UsageQuery = {
        ...
        provider: req.query.provider,
        model: req.query.model,
        groupBy: req.query.groupBy as UsageQuery['groupBy'],
      };
  ...
  app.get("/api/usage/daily", async (req: any, reply: any) => {
    try {
  ...
  app.get("/api/usage/filters", async (req: any, reply: any) => {
    try {
      const { startDate, endDate } = req.query;
      ...
      console.error("Failed to get usage filters:", error);
      reply.status(500).send({ error: error.message || "Failed to get usage filters" });
    }
  });

  // Export usage data
  app.get("/api/usage/export", async (req: any, reply: any) => {
    try {
  ...
  app.delete("/api/usage/cleanup", async (req: any, reply: any) => {
    try {
      const options: CleanupOptions = {
        beforeDate: req.query.beforeDate,
        retentionDays: req.query.retentionDays ? parseInt(req.query.retentionDays) : undefined,
        dryRun: req.query.dryRun === 'true',
      };

      // Default retention days from environment or default to 90
      if (!options.beforeDate && !options.retentionDays) {
        options.retentionDays = parseInt(process.env.USAGE_RETENTION_DAYS || '90');
      }
      ...
      console.error("Failed to cleanup usage:", error);
